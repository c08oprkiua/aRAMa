...
public:
	...
	DirList(const std::string & path, const char *filter = NULL, uint32_t flags = Files | Dirs);
	//!Destructor
	virtual ~DirList();
	//! Load all the files from a directory
	bool LoadPath(const std::string & path, const char *filter = NULL, uint32_t flags = Files | Dirs);
